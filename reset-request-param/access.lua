...
local function transform_querystrings(conf)
  ...
  if conf.rename.querystring then
    ...
    for _, old_name, new_name in iter(conf.rename.querystring) do
      ...
      ngx.log(ngx.ERR,"\n old name is： "..old_name.." new name is: "..new_name.." value type is: "..type(value))
      if type(value) == 'boolean' or querystring.new_name or querystring[new_name] then
        ngx.log(ngx.ERR,"\nquerystring.new_name is not nil \n")
        if type(querystring.new_name) =='table' or type(querystring[new_name]) == 'table' then
          ngx.log(ngx.ERR,"\nnew_name is table \n")
          if type(value)=='table' then
            ngx.log(ngx.ERR,"\nvalue is table.")
            for _,v in ipairs(value) do
              table_insert(querystring[new_name],v)
            end

          else
            ngx.log(ngx.ERR,"\nvalue is not a table.")
            table_insert(querystring[new_name],value)
          end

        else
          ngx.log(ngx.ERR,"\nelse new_name is"..type(querystring.new_name))
          querystring[new_name] = {querystring[new_name],value}
        end
      else
        ngx.log(ngx.ERR,"\nquerystring.new_name is nil\n")
